...
class ankixmis_env
{
    ...
    {/*{{{*/
        global $_G;
		$_G['siteurl'] = preg_replace("/source\/plugin\/ankixmis/i","", $_G['siteurl']);
    ...
    {/*{{{*/
        ...
        if ($charset=='gbk') {
            $sitename = ankixmis_utils::toutf8($sitename);
        }
        return $sitename;
    }/*}}}*/
    ...
    {/*{{{*/
        return self::get_siteurl().'/source/plugin/ankixmis';
    }/*}}}*/
    ...
    {/*{{{*/
        if (!self::$_log_obj) {
            ...
            self::$_log_obj = new ankixmis_log($logcfg);
        }   
    ...
    {/*{{{*/
        if (!isset(self::$_pdo_objs[$key])) {
            $config = C::m("#ankixmis#ankixmis_setting")->get();
            if (!isset($config[$key])) {
            ...
            $pass = $dbconf['pass'];
            $db   = $dbconf['db'];
            self::$_pdo_objs[$key] = new ankixmis_dbo($host,$port,$user,$pass,$db);
        }
