 ...
 * 用户权限表
 **/
class table_ankixmis_auth extends discuz_table
{
    public function __construct() {
		$this->_table = 'ankixmis_auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    public function set()
    {/*{{{*/
        $uid = ankixmis_validate::getNCParameter('uid','uid','integer');
        $auth = ankixmis_validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('ankixmis_auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	{/*{{{*/
        ...
        $auth  = ankixmis_validate::getNCParameter('auth','auth','integer');
        $key   = ankixmis_validate::getNCParameter('key','key','string',128);
        $sort  = ankixmis_validate::getOPParameter('sort','sort','string',1024,'uid');
        $dir   = ankixmis_validate::getOPParameter('dir','dir','string',1024,'ASC');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#ankixmis#ankixmis_usergroup')->grouptitle($row['groupid']);
